...
class JsonItemImplTest {


    private final JsonItemFactory factory = JsonItemFactoryBuilder.standard().build();

    @Test
    public void emptyItemCanPerformAllOperations() {
        JsonItem item = factory.empty();
        ...
        assertThat(item.get(path).getItemType(), equalTo(JsonItem.ItemType.ANY));
        MatcherAssert.assertThat(item.get(path).getPathNode(), JsonPathNodeMatcher.isNodeWith(path, null, null));
    ...
    public void existingListItemCanPerformAllOperations() {
        ...
        JsonItem item = factory.from(jsonElement);
        String path = "somePath";
        ...
        assertThat(item.get(path).getFullPath(), is(equalTo(path)));
        assertThat(item.get(path).getContainer(), equalTo(item));
        assertThat(item.get(path).getItemType(), equalTo(JsonItem.ItemType.LIST));
        MatcherAssert.assertThat(item.get(path).getPathNode(), JsonPathNodeMatcher.isNodeWith(path, null, null));
    ...
    public void existingStructItemCanPerformAllOperations() {
        ...
        assertThat(item.get(path).getFullPath(), is(equalTo(path)));
        assertThat(item.get(path).getContainer(), equalTo(item));
        assertThat(item.get(path).getItemType(), equalTo(JsonItem.ItemType.STRUCT));
        MatcherAssert.assertThat(item.get(path).getPathNode(), JsonPathNodeMatcher.isNodeWith(path, null, null));
    ...
    public void existingStringItemCanPerformAllOperations() {
        ...
        assertThat(item.get(path).getFullPath(), is(equalTo(path)));
        assertThat(item.get(path).getContainer(), equalTo(item));
        assertThat(item.get(path).getItemType(), equalTo(JsonItem.ItemType.DATA));
        MatcherAssert.assertThat(item.get(path).getPathNode(), JsonPathNodeMatcher.isNodeWith(path, null, null));
    ...
    @Test
    public void conversionBetweenNumericTypesWork() {
    ...
    public void canAddAliasesToItem() {
        ...
        JsonItem list = item.get("one/list:0");
        JsonItem list_aliased = item.get("one/list:1=second");
    ...
    @Test
    public void settingFromOtherJsonIon_MakesACopyOfData() {
        JsonItem item = factory.empty();
        JsonItem otherValue = factory.empty();
        JsonItem otherStruct = factory.empty();
        JsonItem otherList = factory.empty();
    ...
    @Test
    public void canSetFieldsFromOtherJsonItem_UsingSelect() {
        JsonElement jsonElement = parseSemiJson("{" +
                "'list':['list_1', 'list_2'], " +
        ...
        JsonItem existing = factory.from(jsonElement);
        JsonItem copy = factory.empty();
        existing.select(
                "string",
    ...
    @Test
    public void canSetFieldsFromOtherJsonItem_UsingSet() {
        JsonElement jsonElement = parseSemiJson("{" +
                "'string':'some_string'" +
                "}");
    ...
    public void canAddToNonExistingList() {
        ...
        JsonItem forList = factory.empty();
    ...
    public void listSpecificMethods() {
        ...
        // list is read-only
        List<JsonItem> list = item.get("list").list();
        ...
        List<JsonItem> updatedList = item.get("list").list();
    ...
    public void structSpecificMethods() {
        ...
        // map is read-only
        Map<String, JsonItem> itemMap = item.get("struct").fields();
        ...
        Map<String, JsonItem> updatedMap = item.get("struct").fields();
    ...
    public void testStrictBehavior_forLists() {
        ...
        JsonItemFactory nonLenientFactory = JsonItemFactoryBuilder.strict().build();
        JsonItem lenient = factory.empty();
        JsonItem nonLenient = nonLenientFactory.empty();
    ...
    public void testStrictBehavior_forJsonStructure_OnExistingValues() {
        ...
        JsonItem lenient = factory.from(jsonElement.deepCopy());
        JsonItem nonLenient = nonLenientFactory.from(jsonElement);
